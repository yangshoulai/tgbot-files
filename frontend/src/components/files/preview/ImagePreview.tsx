...
export function ImagePreview({ file, fullscreen }: PreviewComponentProps) {
  ...
  return (
    <div className={cn("relative grid w-full place-items-center bg-[radial-gradient(circle_at_top,var(--color-primary-soft),transparent_28%),linear-gradient(45deg,#f8fafc_25%,transparent_25%),linear-gradient(-45deg,#f8fafc_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#f8fafc_75%),linear-gradient(-45deg,transparent_75%,#f8fafc_75%)] bg-[length:auto,24px_24px,24px_24px,24px_24px,24px_24px] bg-[position:center,0_0,0_12px,12px_-12px,-12px_0] p-4", fullscreen ? "h-full" : "h-[64vh]") }>
      {!loaded ? (
      ...
      <img
        ...
        className={cn(
          ...
          fullscreen ? "block h-auto max-h-full w-auto max-w-full rounded-xl object-contain shadow-dialog" : "h-full w-full rounded-xl object-contain"
        )}
        loading="lazy"
        onLoad={() => {
          setLoaded(true);
